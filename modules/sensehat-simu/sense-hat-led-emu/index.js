...
const fb_filename = '/run/shm/rpi-sense-emu-screen';
...
function findFB() {
  try {
    fs.accessSync(fb_filename, fs.constants.R_OK | fs.constants.W_OK);
  } catch (err) {
    throw Error('Cannot access ' + fb_filename);
  }
  return fb_filename;
}
...
function getGamma() {
  ...
  const fd = fs.openSync(fb, 'r');
  fs.readSync(fd, buffer, 0, 32, 128);
  fs.closeSync(fd);
...
function setGamma(GammaArray) {
  ...
  const fd = fs.openSync(fb, 'r+');
  fs.writeSync(fd, Buffer.from(GammaArray), 0, 32, 128);
  fs.closeSync(fd);
}
...
function gammaReset() {
  ...
  const fd = fs.openSync(fb, 'r+');
  fs.writeSync(fd, Buffer.from(GammaDefault), 0, 32, 128);
  fs.closeSync(fd);
}
...
function setLowLight(bol) {
  const fd = fs.openSync(fb, 'r+');
  if (bol) {
    fs.writeSync(fd, Buffer.from(GammaLow), 0, 32, 128);
  } else {
    fs.writeSync(fd, Buffer.from(GammaDefault), 0, 32, 128);
  }
  fs.closeSync(fd);
